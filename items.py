...
ITEM_ENUM_TO_DATA: dict[ItemName, item_data.ItemData] = {data.name: data for data in item_data.ITEM_DATA if data.code is not None}
...
ITEM_GROUP_MAP: dict[str, set[str]] = {group: {item.name.value for item in item_data.ITEM_DATA if group in item.groups} for group in ITEM_GROUPS}
